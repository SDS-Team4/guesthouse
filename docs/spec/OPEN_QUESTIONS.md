...
### BD-09 관리자 접근 제한 baseline
- Source: former `OQ-05`
- Status: finalized baseline decision
- Decision: 관리자 접근은 role check만으로 끝내지 않고, `M8`에서 ingress 또는 VPN 기반 IP allowlist를 baseline으로 적용한다.
- Why: SRS의 `REQ-SEC-006`, `REQ-SEC-007`, Appendix E `T7`, `T8`, `T9`는 관리자 영역에 더 강한 접근통제를 요구한다.
- Operational consequence: 관리자 API는 네트워크 경계와 애플리케이션 감사를 함께 사용해야 하며, 단순 로그인 성공만으로 인터넷 전체에 노출하지 않는다.
- Affected milestones: `M0`, `M7`, `M8`

### BD-10 파일 저장 및 다운로드 baseline
- Source: former `OQ-06`
- Status: finalized baseline decision
- Decision: V1 파일 업로드는 오브젝트 스토리지 + 메타데이터 테이블 모델을 baseline으로 한다. 저장 파일명은 UUID로 생성하고, 직접 공개 URL이 아니라 권한 검증 후 간접 다운로드를 사용한다.
- Why: `REQ-SEC-010`, `REQ-SEC-011`, `REQ-SEC-012`와 관리자 공지/첨부 운영 범위를 함께 만족시키려면 로컬 파일 시스템보다 권한/검증/운영 분리가 쉬운 구조가 필요하다.
- Security consequence: 허용 확장자, MIME, magic number 검증과 저장명 UUID화, 권한 기반 다운로드를 기본 파이프라인으로 본다.
- Affected milestones: `M0`, `M5`, `M7`, `M8`
